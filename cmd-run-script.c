...
/*
 * Runs a script without a window, passing arguments to it.
 */

static enum cmd_retval		cmd_run_script_exec(struct cmd *,
				    struct cmdq_item *);

static void	cmd_run_script_timer(int, short, void *);
static void	cmd_run_script_callback(struct job *);
static void	cmd_run_script_free(void *);
static void	cmd_run_script_print(struct job *, const char *);
...
const struct cmd_entry cmd_run_script_entry = {
	...
	.flags = 0,
	.exec = cmd_run_script_exec
};

struct cmd_run_script_data {
...
static void
cmd_run_script_print(struct job *job, const char *msg)
{
	struct cmd_run_script_data	*cdata = job_get_data(job);
...
cmd_run_script_exec(struct cmd *self, struct cmdq_item *item)
{
	struct args			*args = cmd_get_args(self);
	struct cmd_find_state		*target = cmdq_get_target(item);
	struct cmd_run_script_data	*cdata;
	...
	evtimer_set(&cdata->timer, cmd_run_script_timer, cdata);
	if (delay != NULL) {
		timerclear(&tv);
		tv.tv_sec = (time_t)d;
...
static void
cmd_run_script_timer(__unused int fd, __unused short events, void* arg)
{
	struct cmd_run_script_data	*cdata = arg;
	...
	if (cmd == NULL) {
		...
		cmd_run_script_free(cdata);
		return;
	}
	if (job_run(cmd, 0, NULL, NULL, cdata->s, cdata->cwd, NULL,
	    cmd_run_script_callback, cmd_run_script_free, cdata,
...
static void
cmd_run_script_callback(struct job *job)
{
	struct cmd_run_script_data	*cdata = job_get_data(job);
...
static void
cmd_run_script_free(void *data)
{
	struct cmd_run_script_data	*cdata = data;
